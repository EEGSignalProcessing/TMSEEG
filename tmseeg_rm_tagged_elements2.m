...
% tmseeg_rm_tagged_elements2() - removed selected trials, channels, and
% trial/channel pairings according to the toDelete.mat fule generated in
...
% threshold VARS.NUM_BAD_CHANS_2, the corresponding trial is deleted
bt             = (sum(~toDelete) - numel(full_ch_rm))   > VARS.NUM_BAD_CHANS_2;
toDelete(:,bt) = [];

% If the number of trials marked for deletion in a channel exceeds the set
% threshold VARS.NUM_BAD_TRIAL_2, the corresponding channel is deleted
bc             = sum(~toDelete,2)> VARS.NUM_BAD_TRIALS_2;
toDelete(bc,:) = [];
labels(bc) = [];
...
%removal (set by threshold VARS.NUM_BAD_CHANS_2 and visa versa
if any(~toDelete(:))
    EEGkk   = EEG;
    x       = find(any(~toDelete,2));
